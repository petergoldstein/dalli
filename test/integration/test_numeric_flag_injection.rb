...
describe 'numeric flag injection' do
  MemcachedManager.supported_protocols.each do |p|
    describe "using the #{p} protocol" do
      ...
      it 'rejects fetch_with_lock TTLs that would inject commands' do
        memcached_persistent(p) do |dc|
          dc.flush
          dc.set('victim', 'alive')

          assert_raises(ArgumentError) { dc.fetch_with_lock('k', ttl: 60, lock_ttl: "30\r\nflush_all\r\n") { 'v' } }
          assert_raises(ArgumentError) do
            dc.fetch_with_lock('k', ttl: 60, recache_threshold: "5\r\nflush_all\r\n") { 'v' }
          end

          assert_equal 'alive', dc.get('victim')
        end
      end

      it 'still accepts Integer and decimal String defaults' do
        memcached_persistent(p) do |dc|
          dc.flush
